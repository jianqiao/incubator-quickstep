...
struct AggregationContext<
    kSum, value_tid, std::enable_if_t<
        meta::EqualsAnyValue<TypeID, value_tid,
                             kDecimal2, kDecimal4, kDecimal6>::value>> {
  using StateT = std::int64_t;
  using AtomicStateT = std::atomic<StateT>;
...
template <TypeID value_tid>
struct AggregationUnsafeDecimalSumOps {
  using ValueT = typename TypeIDTrait<value_tid>::cpptype;
  using StateT = typename AggregationContext<kSum, value_tid>::StateT;
  using ResultT = typename TypeIDTrait<
      AggregationContext<kSum, value_tid>::kResultTypeID>::cpptype;
...
template <>
struct AggregationUnsafeOps<kSum, kDecimal2> : AggregationUnsafeDecimalSumOps<kDecimal2> {};
template <>
struct AggregationUnsafeOps<kSum, kDecimal4> : AggregationUnsafeDecimalSumOps<kDecimal4> {};
template <>
struct AggregationUnsafeOps<kSum, kDecimal6> : AggregationUnsafeDecimalSumOps<kDecimal6> {};
...
template <TypeID value_tid>
struct AggregationAtomicDecimalSumOps {
  using ValueT = typename TypeIDTrait<value_tid>::cpptype;
  using AtomicStateT = typename AggregationContext<kSum, value_tid>::AtomicStateT;
  using ResultT = typename TypeIDTrait<
      AggregationContext<kSum, value_tid>::kResultTypeID>::cpptype;
  ...
  static inline void MergeValueAtomic(AtomicStateT *state, const ValueT &value) {
    state->fetch_add(value.data, std::memory_order_relaxed);
  }
  static inline void MergeStateAtomic(AtomicStateT *dst, const AtomicStateT &src) {
    dst->fetch_add(src.load(std::memory_order_relaxed),
                   std::memory_order_relaxed);
  ...
  static inline void FinalizeAtomic(ResultT *result, const AtomicStateT &state) {
    result->data = state.load(std::memory_order_relaxed);
  }
};

template <>
struct AggregationAtomicOps<kSum, kDecimal2> : AggregationAtomicDecimalSumOps<kDecimal2> {};
template <>
struct AggregationAtomicOps<kSum, kDecimal4> : AggregationAtomicDecimalSumOps<kDecimal4> {};
template <>
struct AggregationAtomicOps<kSum, kDecimal6> : AggregationAtomicDecimalSumOps<kDecimal6> {};
