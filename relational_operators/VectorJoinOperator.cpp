...
bool VectorJoinOperator::getAllWorkOrders(
  ...
  switch (join_type_) {
    case JoinType::kInnerJoin:
      return getAllNonOuterJoinWorkOrders<VectorInnerJoinWorkOrder>(
          container, query_context, storage_manager);
    case JoinType::kLeftSemiJoin:
      return getAllNonOuterJoinWorkOrders<VectorSemiJoinWorkOrder>(
          container, query_context, storage_manager);
    case JoinType::kLeftAntiJoin:
      return getAllNonOuterJoinWorkOrders<VectorAntiJoinWorkOrder>(
          container, query_context, storage_manager);
    case JoinType::kLeftOuterJoin:
      return getAllOuterJoinWorkOrders(
          container, query_context, storage_manager);
    default:
      LOG(FATAL) << "Unknown join type in VectorJoinOperator::getAllWorkOrders()";
  }
  return false;
}

template <class JoinWorkOrderClass>
bool VectorJoinOperator::getAllNonOuterJoinWorkOrders(
    WorkOrdersContainer *container,
    QueryContext *query_context,
    StorageManager *storage_manager) {
    ...
    if (probe_relation_is_stored_) {
      ...
      for (std::size_t part_id = 0; part_id < num_partitions_; ++part_id) {
        const CollisionFreeVector &hash_table =
            *(query_context->getCollisionFreeVector(hash_table_index_, part_id));
...
bool VectorJoinOperator::getAllOuterJoinWorkOrders(
    WorkOrdersContainer *container,
    QueryContext *query_context,
    StorageManager *storage_manager) {
  // We wait until the building of global hash table is complete.
  if (blocking_dependencies_met_) {
    ...
    if (probe_relation_is_stored_) {
      ...
      for (std::size_t part_id = 0; part_id < num_partitions_; ++part_id) {
        ...
        for (const block_id probe_block_id : probe_relation_block_ids_[part_id]) {
          container->addNormalWorkOrder(
              new VectorOuterJoinWorkOrder(query_id_, build_relation_, probe_relation_, join_key_attributes_,
...
bool VectorJoinOperator::getAllWorkOrderProtos(WorkOrderProtosContainer *container) {
  switch (join_type_) {
    /*
    case JoinType::kInnerJoin:
    ...
    case JoinType::kLeftOuterJoin:
      ...
      */
    default:
      LOG(FATAL) << "Unknown join type in VectorJoinOperator::getAllWorkOrderProtos()";
  }
}

/*
bool VectorJoinOperator::getAllNonOuterJoinWorkOrderProtos(
...
serialization::WorkOrder* VectorJoinOperator::createNonOuterJoinWorkOrderProto(
    const serialization::HashJoinWorkOrder::HashJoinWorkOrderType hash_join_type,
...
bool VectorJoinOperator::getAllOuterJoinWorkOrderProtos(WorkOrderProtosContainer *container) {
  return false;
  /*
  // We wait until the building of global hash table is complete.
  if (!blocking_dependencies_met_) {
...
serialization::WorkOrder* VectorJoinOperator::createOuterJoinWorkOrderProto(const block_id block,
...
void VectorInnerJoinWorkOrder::execute() {
...
void VectorInnerJoinWorkOrder::executeWithoutCopyElision(ValueAccessor *probe_accessor) {
...
void VectorInnerJoinWorkOrder::executeWithCopyElision(ValueAccessor *probe_accessor) {
...
void VectorSemiJoinWorkOrder::execute() {
  output_destination_->setInputPartitionId(part_id_);
...
void VectorSemiJoinWorkOrder::executeWithResidualPredicate() {
  const relation_id build_relation_id = build_relation_.getID();
  const relation_id probe_relation_id = probe_relation_.getID();
...
void VectorSemiJoinWorkOrder::executeWithoutResidualPredicate() {
...
void VectorAntiJoinWorkOrder::executeWithoutResidualPredicate() {
  DCHECK(residual_predicate_ == nullptr);
...
void VectorAntiJoinWorkOrder::executeWithResidualPredicate() {
  const relation_id build_relation_id = build_relation_.getID();
  const relation_id probe_relation_id = probe_relation_.getID();
...
void VectorOuterJoinWorkOrder::execute() {
  output_destination_->setInputPartitionId(part_id_);
  ...
  if (num_tuples_without_matches > 0) {
    ...
    for (std::size_t i = 0; i < selection_.size(); ++i) {
      if (is_selection_on_build_[i]) {
        ...
        // In that case, this VectorOuterJoinWorkOrder needs to be updated to
