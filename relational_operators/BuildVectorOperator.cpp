...
bool BuildVectorOperator::getAllWorkOrders(
  ...
  if (input_relation_is_stored_) {
    ...
    for (std::size_t part_id = 0; part_id < num_partitions_; ++part_id) {
      CollisionFreeVector *hash_table = query_context->getCollisionFreeVector(hash_table_index_, part_id);
      for (const block_id block : input_relation_block_ids_[part_id]) {
        container->addNormalWorkOrder(
            new BuildVectorWorkOrder(query_id_, input_relation_, join_key_attributes_,
                                     any_join_key_attributes_nullable_, part_id, block, hash_table, storage_manager,
...
bool BuildVectorOperator::getAllWorkOrderProtos(WorkOrderProtosContainer *container) {
  if (input_relation_is_stored_) {
    if (started_) {
      return true;
    }
...
serialization::WorkOrder* BuildVectorOperator::createWorkOrderProto(const block_id block, const partition_id part_id) {
  return nullptr;
  /*
  serialization::WorkOrder *proto = new serialization::WorkOrder;
  proto->set_work_order_type(serialization::BUILD_VECTOR);
  proto->set_query_id(query_id_);

  proto->SetExtension(serialization::BuildVectorWorkOrder::relation_id, input_relation_.getID());
  for (const attribute_id attr_id : join_key_attributes_) {
    proto->AddExtension(serialization::BuildVectorWorkOrder::join_key_attributes, attr_id);
  }
  proto->SetExtension(serialization::BuildVectorWorkOrder::any_join_key_attributes_nullable,
                      any_join_key_attributes_nullable_);
  proto->SetExtension(serialization::BuildVectorWorkOrder::join_hash_table_index, hash_table_index_);
  proto->SetExtension(serialization::BuildVectorWorkOrder::partition_id, part_id);
  proto->SetExtension(serialization::BuildVectorWorkOrder::block_id, block);
  proto->SetExtension(serialization::BuildVectorWorkOrder::lip_deployment_index, lip_deployment_index_);

  for (const QueryContext::lip_filter_id lip_filter_index : lip_filter_indexes_) {
    proto->AddExtension(serialization::BuildVectorWorkOrder::lip_filter_indexes, lip_filter_index);
  }
...
void BuildVectorWorkOrder::execute() {
  ...
  DCHECK_EQ(1u, join_key_attributes_.size());
  CHECK(hash_table_->putValueAccessor(accessor.get(),
                                      join_key_attributes_.front(),
                                      any_join_key_attributes_nullable_,
                                      &generator)
            == HashTablePutResult::kOK)
