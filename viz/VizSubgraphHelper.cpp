...
void VizSubgraphHelper::groupingSubgraphs(std::vector<std::unique_ptr<VizConfig>> &confs, json& viz) {
...
std::unordered_map<std::string, bool> VizSubgraphHelper::checkGroup
        (const std::unordered_map<std::string, std::vector<std::unique_ptr<VizConfig>>> &group) {
  std::unordered_map<std::string, int> measure_size;
  int max_size = -1;
  for (const auto &pair : group) {
    int sz = 0;
    for (const auto &conf : pair.second)
