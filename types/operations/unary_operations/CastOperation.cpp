...
// Get target type name from CastOperation's argument signature.
std::string GetTargetTypeNameFromSignature(const OperationSignaturePtr &signature) {
...
// Get target type from CastOperation's argument signature.
const Type* ParseTargetTypeFromSignature(const OperationSignaturePtr &signature) {
  const std::string type_name = GetTargetTypeNameFromSignature(signature);
...
std::size_t CastOperation::getOperatorPrecedence() const {
  return kOperatorPrecedenceCast;
}

std::string CastOperation::formatExpression(const OperationSignaturePtr &signature,
...
bool CastOperation::canApplyToSignature(const OperationSignaturePtr &signature,
  ...
  DCHECK_EQ(2u, signature->getNumParameters());
  const std::string target_type_name = GetTargetTypeNameFromSignature(signature);
  const Type *target_type = TypeFactory::ParseTypeFromString(target_type_name);
...
const Type& CastOperation::resultTypeForSignature(
  ...
  return *ParseTargetTypeFromSignature(signature);
}
...
UncheckedUnaryOperator* CastOperation::makeUncheckedUnaryOperatorForSignature(
  ...
  DCHECK(UnaryOperation::canApplyToSignature(signature));
  const Type &source_type = signature->getParameterType(0);
  const Type &target_type = *ParseTargetTypeFromSignature(signature);
