...
#include "types/operations/unary_operations/CxxMathUnaryOperations.hpp"
...
#include "types/operations/binary_operations/BinaryOperationCodeGen.hpp"
#include "types/operations/binary_operations/CxxMathBinaryOperations.hpp"
...
const UnaryOperation& OperationFactory::GetUnaryOperation(
    const OperationSignatureLitePtr &signature) {
  const Operation &operation = GetOperation(signature);
  DCHECK(operation.getOperationSuperTypeID() == Operation::kUnaryOperation);
  return static_cast<const UnaryOperation&>(operation);
}

const BinaryOperation& OperationFactory::GetBinaryOperation(
    const OperationSignatureLitePtr &signature) {
  const Operation &operation = GetOperation(signature);
  DCHECK(operation.getOperationSuperTypeID() == Operation::kBinaryOperation);
  return static_cast<const BinaryOperation&>(operation);
}
