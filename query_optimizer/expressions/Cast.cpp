...
std::pair<std::string, std::size_t> Cast::generateNameWithPrecedence() const {
  ...
  const std::string name = operation.formatExpression(signature,
                                                      operand_info.first,
                                                      operand_info.second);
  return std::make_pair(name, operation.getOperatorPrecedence());
}
