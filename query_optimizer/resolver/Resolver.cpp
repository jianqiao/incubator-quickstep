    ...
    case ParseSelectionClause::kNonStar: {
      ...
      for (const ParseSelectionItem &selection_item :
        ...
        if (parse_alias != nullptr) {
          ...
          // Create an Alias if the expression is not a named expression.
          if (project_named_expression == nullptr) {
            const std::string generated_name = project_scalar->generateNameWithPrecedence().first;
            project_named_expression = E::Alias::Create(project_expression_id,
                                                        project_scalar,
                                                        "" /* attribute_name */,
                                                        generated_name,
...
expressions::ScalarPtr Resolver::resolveScalarFunction(
  ...
  switch (operation.getOperationSuperTypeID()) {
    ...
    case Operation::kBinaryOperation: {
      DCHECK_EQ(2u, regular_arguments.size());
      return E::BinaryExpression::Create(
          resolved_signature,
          static_cast<const BinaryOperation&>(operation),
          regular_arguments[0],
          regular_arguments[1]);
    }
    default:
